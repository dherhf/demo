...
const handleDeskChange = (event) => {
  deskError.value = '';
  // 可以在此处添加验证逻辑
  if (event.target.value && !desks.value.some(d => d.id === event.target.value)) {
...
const handleCustomerChange = (event) => {
  customerError.value = '';
  // 可以在此处添加验证逻辑
  if (event.target.value && !customers.value.some(c => c.id === event.target.value)) {
...
const handleEmployeeChange = (event) => {
  employeeError.value = '';
  // 可以在此处添加验证逻辑
  if (event.target.value && !employees.value.some(e => e.id === event.target.value)) {
...
onMounted(() => {
  ...
  axios.get('http://localhost:8080/api/desk/open')
    .then(response => {
