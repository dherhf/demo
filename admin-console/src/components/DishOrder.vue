  ...
  <div class="flex h-full bg-gray-50 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col p-6 space-y-6 overflow-y-auto">
      ...
      <div class="bg-white shadow rounded p-4">
        ...
        <div v-if="orderItem.length === 0" class="text-gray-500">
          暂无菜品，请选择菜品
        </div>
        <div v-else>
          <table class="w-full">
...
const submitOrder = async () => {
  try {
    // 获取当前选中的桌台、客户和员工信息
    // 通过组件引用获取数据
    const { desk, customer, employee } =
      orderComponent.value?.getSelectedData() || {};
    ...
    for (const item of orderItem.value) {
      ...
      await axios.post("http://localhost:8080/api/order-item", orderItemData, {
        headers: {
          "Content-Type": "application/json",
        },
      });
