          ...
          <template #default="{ row }">
            ...
            >
              <svg
                ...
                stroke="currentColor"
                class="w-4 h-4"
                :class="{ 'animate-spin': deletingIds.has(row.id) }"
              >
                <path
                  v-if="!deletingIds.has(row.id)"
    ...
    >
      ...
      <div class="flex gap-2">
        ...
        <button
          v-if="!isEditing && !isAdding"
          @click="addNewRow"
          class="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white focus:outline-none transition-colors duration-200 flex items-center"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
    ...
    const toSave =
      recordsToSave.length > 0
        ...
        : customers.value.filter((customer) =>
            editedRows.value.has(customer.id)
          );
