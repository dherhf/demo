  ...
  <div class="flex h-screen bg-gray-50 relative z-0">
    ...
    <div class="w-64 bg-white shadow-lg p-4 flex flex-col">
      ...
      <ul v-else class="space-y-2 overflow-y-auto">
        <li
            v-for="category in dishCategory" :key="category.id"
      ...
      <div class="mb-8 bg-white rounded-lg shadow-md overflow-hidden">
        <table class="w-full">
          ...
          <tr v-if="currentCategory">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ currentCategory.tags }}</td>
      ...
      <div class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tr v-for="dish in filteredDishes" :key="dish.id" class="hover:bg-gray-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 flex space-x-4">
                <button @click="openDishModal(dish)" class="text-blue-600 hover:underline hover:text-blue-800">
                  编辑
                </button>
                <button @click="deleteDish(dish.id)" class="text-red-600 hover:underline hover:text-red-800">
                  删除
                </button>
              </td>
            </tr>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 transform transition-all">
        ...
        <div class="space-y-4">
          ...
          <div>
            <label for="categoryDescription" class="block text-sm font-medium text-gray-700 mb-1">分类描述</label>
            <textarea
    ...
    >
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 transform transition-all">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium text-gray-900">{{ isEdit ? '编辑菜品' : '添加菜品' }}</h3>
          <button @click="showDishModal = false" class="text-gray-400 hover:text-gray-500">
            <i class="fa fa-times"></i>
          </button>
        </div>
...
const addCategory = async () => {
  try {
    // 发送数据到后端
    const response = await axios.post("http://localhost:8080/api/dish-category", {
...
const filteredDishes = computed(() =>
    currentCategory.value
        ? dishes.value.filter(dish => dish.categoryId === currentCategory.value.id)
        : []
)
...
const saveDish = async () => {
  ...
  try {
    let response
    if (isEdit.value) {
      response = await axios.put(`http://localhost:8080/api/dish/${currentDish.value.id}`, {
      ...
      const dishIndex = dishes.value.findIndex(d => d.id === currentDish.value.id)
      if (dishIndex !== -1) {
    ...
    error.value = err.message || (isEdit.value ? '编辑菜品失败' : '添加菜品失败')
