    ...
    <div class="overflow-x-auto">
      <table class="min-w-full bg-white rounded-lg overflow-hidden">
        ...
        <tr v-for="employee in filteredEmployees" :key="employee.id"
          ...
          <td class="px-6 py-4 text-sm text-gray-800">{{ employee.phone }}</td>
        ...
        <tr v-if="!employees.length">
          <td colspan="5" class="px-6 py-4 text-center text-gray-500">暂无员工数据</td>
        </tr>
        <tr v-else-if="!filteredEmployees.length">
          <td colspan="5" class="px-6 py-4 text-center text-gray-500">未找到匹配的员工</td>
        </tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 transform transition-all duration-300 scale-100">
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-800">{{ isEditing ? '编辑员工' : '添加员工' }}</h3>
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 mb-1" for="phone">电话</label>
              <input
                  type="tel"
                  id="phone"
    ...
    <div v-if="deleteModalVisible" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 transform transition-all duration-300 scale-100">
        <div class="p-6">
          ...
          <p class="text-gray-600 mb-6">你确定要删除这名员工吗？此操作不可撤销。</p>
...
const submitForm = async () => {
  ...
  if (isEditing.value) {
    // 更新员工信息
    response = await axios.put(`http://localhost:8080/api/employee/${formData.id}`, {
      id: formData.id,
      name: formData.name,
      age: formData.age,
