  ...
  <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
    <div class="p-6 border-b border-gray-200">
      ...
      <div class="flex flex-wrap items-center mt-2 text-sm text-gray-600">
        <div class="mr-4">
          <i class="fa fa-calendar-o mr-1"></i> {{ order.createDate }}
        </div>
        <div class="mr-4">
          <i class="fa fa-user-o mr-1"></i> 顾客ID: {{ order.customerId }}
        </div>
        <div class="mr-4">
          <i class="fa fa-cutlery mr-1"></i> 桌号: {{ order.deskId }}
        </div>
        <div class="mr-4">
          <i class="fa fa-user mr-1"></i> 服务员: {{ order.employeeId }}
        </div>
        <div
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
                ...
                <div class="flex items-center">
                  ...
                  <div class="ml-4">
                    <div class="text-sm font-medium text-gray-900">
                      {{ getDishName(item.dishId) || "未知菜品" }}
                    </div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ¥{{ getDishPrice(item.dishId) || 0 }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ item.quantity }}
              </td>
              ...
              >
                ¥{{ (getDishPrice(item.dishId) || 0) * item.quantity }}
              </td>
            </tr>
          </tbody>
        </table>
...
const fetchOrderDetails = async () => {
  ...
  try {
    ...
    const orderItemsResponse = await axios.get(
      `http://localhost:8080/api/order-item/order/${orderId}`
    );
